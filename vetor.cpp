...
//<------------- FUNÇÕES EXTRAS ---------->
template <typename T> void Vetor<T>::clear() {
    while (ptr) {
        Node<T>* temp = ptr;
        ptr = ptr->next;
        delete temp;
...
template <typename T>
void bubbleSort(T* array, int size, bool (*func_compara)(T, T)) {
...
//<------------- FIM DAS FUNÇÕES EXTRAS ---------->

template <typename T> Vetor<T>::Vetor()  : ptr(nullptr), tamanho(0) {}
...
template <typename T> void Vetor<T>::show() const {
    Node<T>* temp = ptr;
    while (temp) {
...
template <typename T> T Vetor<T>::at(int i) const {
    if (i < 0 || i >= tamanho) throw std::out_of_range("Esse índice não existe!");
...
template <typename T> bool Vetor<T>::sort(bool (*func_compara)(T, T)) {
